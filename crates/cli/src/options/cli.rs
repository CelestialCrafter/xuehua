use std::{collections::VecDeque, env, fmt::Write, path::PathBuf};
...
impl From<Flags> for [&'static str; 2] {
    fn from(value: Flags) -> Self {
        // pico-args accepts "" for empty short flags
        [value.short.unwrap_or(""), value.long]
    }
}
...
pub fn assemble_spec<'a>(
    path: impl DoubleEndedIterator<Item = &'a ArgumentDefinition>,
) -> VecDeque<SpecFragment> {
    let (lower, _) = path.size_hint();
    let mut fragments = VecDeque::with_capacity(lower);

    for argument in path.flat_map(with_children).rev() {
        match argument {
...
pub fn format_spec<'a>(spec: impl ExactSizeIterator<Item = &'a SpecFragment>) -> String {
...
pub fn format_help<'a>(help: impl Iterator<Item = &'a HelpFragment> + Clone) -> String {
    let argument_padding = {
        let lengths = help.clone().map(|fragment| match fragment {
            HelpFragment::ArgumentSegment { segment, .. } => segment.len(),
            _ => 0,
        });
    ...
    for fragment in help {
        match fragment {
            ...
            HelpFragment::ArgumentSegment {
                ...
                if *last {
                    let _ = write!(msg, "{segment:argument_padding$}");
                } else {
                    let _ = write!(msg, "{segment}, ");
                }
            }
            HelpFragment::Description { last, text } => {
                let _ = writeln!(msg, " {text}");
                if *last {
                    msg += "\n";
                }
            }
            HelpFragment::About { program, version } => {
                let _ = write!(msg, "{program} v{version}");
            }
        }
    }
...
mod tests {
    ...
    fn test_spec() {
        ...
        if action.is_err() || help {
            ...
            let spec = assemble_spec(path.iter());
            let spec_msg = format_spec(spec.iter());

            let help = assemble_help(&path);
            let help_msg = format_help(help.iter());
