...
pub struct BubblewrapBuilder<'a> {
    output: &'a Path,
    child: Option<(Child, ChildStdin, BufReader<ChildStdout>)>,
}

impl<'a> BubblewrapBuilder<'a> {
    pub fn new(output: &'a Path) -> Result<Self, BuilderError> {
        Ok(Self {
            child: None,
            output,
        })
...
impl Builder for BubblewrapBuilder<'_> {
...
impl Drop for BubblewrapBuilder<'_> {
    fn drop(&mut self) {
