...
use crate::modules::executor::{Executor, ExecutorError};
...
impl CommandResponse {
    fn extract(self) -> Result<CommandResponseInfo, ExecutorError> {
        self.info
...
impl Executor for BubblewrapExecutor {
    ...
    fn run(&mut self, command: &process::Command) -> Result<process::Output, ExecutorError> {
        let child = self.init.as_mut().ok_or(ExecutorError::Uninitialized)?;
